...
my $input_f = file (__FILE__)->dir->parent->file ('local', 'iana-idna', "$version.xml");
my $doc = new Web::DOM::Document;
my $parser = new Web::XML::Parser;
$parser->parse_char_string ((decode 'utf-8', scalar $input_f->slurp) => $doc);

my $data_d = file (__FILE__)->dir->parent->subdir ('src', 'set', 'idna-tables-' . $version);
$data_d->mkpath;
...
if ($doc->document_element->local_name eq 'redirect') {
  ...
  $reg =~ s/^idna-tables-//;
  exec 'make', "src/set/idna-tables-$reg/files", 'UNICODE_VERSION=' . $reg;
}

for (@{($doc->get_element_by_id ('idna-tables-properties') ||
        $doc->get_element_by_id ('idna-tables-'.$version.'-properties'))
...
unless ($Sets->{CONTEXT}) {
  ...
  print { $f->openw } sprintf '$idna-tables-%s:CONTEXTJ | $idna-tables-%s:CONTEXTO', $version, $version;
}
